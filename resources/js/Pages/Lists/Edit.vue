...
<template>
  ...
  <AppLayout :breadcrumbs="[
    ...
    { label: list.name, href: route('lists.show', list.id) },
    { label: 'Edit' },
  ]">
    <PageHeader :title="`Edit: ${list.name}`" subtitle="Update the list details.">
      <template #actions>
        <Link :href="route('lists.index')" class="text-sm text-muted hover:text-foreground transition-colors">
          ← Back to Lists
        </Link>
        <Button variant="outline" @click="$inertia.visit(route('lists.show', list.id))">Cancel</Button>
    ...
    <form @submit.prevent="submit" class="max-w-2xl space-y-6">
      ...
      <div class="flex items-center justify-end gap-3">
        <Link :href="route('lists.show', list.id)" class="text-sm text-muted hover:text-foreground transition-colors">
          Cancel
        </Link>
        <Button :loading="form.processing" @click="submit">
          <Save class="h-4 w-4" />
